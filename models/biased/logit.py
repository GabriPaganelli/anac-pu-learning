"""
Biased Learning -- Regressione Logistica (Elastic Net)              [biased/logit.py]
U trattato come classe negativa con peso ridotto W_UNLABELED.
Baseline lineare interpretabile, analoga a nnpu_logit.py
ma con loss standard (cross-entropy pesata, non stimatore nnPU).

Pesi (biased learning):
  PNPU=True  (PNPU): P -> 1.0, N certi -> 1.0,           U -> W_UNLABELED
  PNPU=False (PU):   P -> 1.0, N certi -> W_UNLABELED,   U -> W_UNLABELED

Penalità: Elastic Net con l1_ratio=ELASTIC_NET_ALPHA fisso (0.5).
Iperparametro tunato: C (inverso della regolarizzazione).
...
# Logit -- penalità Elastic Net
ELASTIC_NET_ALPHA = 0.5    # l1_ratio in sklearn (fisso -- non tunato)
MAX_ITER          = 2000   # iterazioni massime per saga

# Griglia su C (inverso della forza di regolarizzazione; più alto = meno regolarizzato)
...
def select_C(
    ...
    Allena LogisticRegression per ogni C in C_GRID.
    Ritorna il C con miglior lift@1% sull'inner val.
    """
    n = len(X_tr)
    ...
    if n_p_iva == 0:
        print(f"    [WARN] inner val senza positivi -- uso default C={C_GRID[2]}")
        return C_GRID[2]

    # Scaler fittato solo sull'inner train
    ...
    best_C, best_lift = C_GRID[0], -np.inf
    t0 = time.time()
    ...
    for C in C_GRID:
        ...
        clf = LogisticRegression(
            penalty      = "elasticnet",
            solver       = "saga",
            l1_ratio     = ELASTIC_NET_ALPHA,
            C            = C,
            max_iter     = MAX_ITER,
            random_state = RANDOM_SEED,
        )
        clf.fit(X_itr_s, y_itr, sample_weight=w_itr)
        proba      = clf.predict_proba(X_iva_s)[:, 1]
        sp, sn, su = get_group_scores(proba, label_iva)
        lft        = lift_at_k(sp, sn, su, k=0.01)
        print(f"      C={C:<8}  lift@1%={lft:.3f}  "
              f"({int(time.time()-t_c)}s -- {int(time.time()-t0)}s totali)")
        if lft > best_lift:
            best_lift, best_C = lft, C

    print(f"    => C*={best_C}  (inner lift@1%={best_lift:.3f})")
    return best_C
...
def run_cv(df: pd.DataFrame, features: list) -> pd.DataFrame:
    ...
    for k in range(N_OUTER_FOLDS):
        ...
        # -- Inner validation: selezione C -------------------------------------
        print(f"  [{_ts()}] Inner validation per selezione C (l1_ratio={ELASTIC_NET_ALPHA})...")
        best_C = select_C(X_tr, y_tr, w_tr, label_tr, rng)

        # -- Scaler finale fittato su tutto il training -------------------------
        print(f"  [{_ts()}] Scaling e fit finale (C={best_C})...")
        t_fit   = time.time()
...
def main():
    ...
    print(f"  Biased Logit  |  M{MODEL_NUMBER}  |  variante={variant.upper()}")
    print(f"  Elastic Net: l1_ratio={ELASTIC_NET_ALPHA} (fisso)  solver=saga")
    print(f"  C grid: {C_GRID}  |  W_UNLABELED={W_UNLABELED}")
    ...
    out_csv = Path(OUT_DIR) / f"logit_M{MODEL_NUMBER}_{variant}_fold_metrics.csv"
